...
import (
	...
	livestream "github.com/bluenviron/mediamtx/internal/liveStream"
...
func (pa *path) runInner() error {
	for {
		...
		case req := <-pa.chRemovePublisher:
			pa.Log(logger.Info, "publishing stop for %s", pa.Name())
		...
		case req := <-pa.chStartPublisher:
			pa.Log(logger.Info, "publishing start for %s", pa.Name())
...
func (pa *path) doRemovePublisher(req defs.PathRemovePublisherReq) {
	...
	// stop transcode of rtmp hls
	liveStream := livestream.GetLiveStreamInstance(pa.name)
	err := liveStream.StopPublishToTranscoder()

	if err != nil {
		pa.Log(logger.Error, "error while stopping live stream transcode: %v", err)
	}
...
func (pa *path) setReady(desc *description.Session, allocateEncoder bool) error {
	...
	pa.Log(logger.Info, "stream created: %s \n", pa.name)

	var err error

	// rtmp to transcode
	liveStream := livestream.NewLiveStream(pa.name)
	err = liveStream.StartPublishToTranscoder()
	if err != nil {
		return err
	}
